...
// === CHATBOT 3.5 (CÉREBRO LOCAL + AUTO-CORREÇÃO DE SESSÃO) ===
// Correção: Lógica de Retry automático para Erro 500 (Sessão Inválida no Render)
...
(function() {
    ...
    async function send() {
        ...
        try {
            ...
            // LÓGICA DE AUTO-CORREÇÃO DE ERRO 500 (SESSÃO EXPIRADA NO SERVIDOR)
            if (res.status === 500) {
                console.warn("[Auto-Repair] Erro 500 detectado (possível reinício do servidor). Limpando sessão e retentando...");
                sessionId = null; 
                localStorage.removeItem('chat_sess_id');
                // Retenta imediatamente com sessão limpa
            ...
            document.getElementById('typing')?.remove();
            // Só reseta ID se for erro de rede, para não prender o usuário em loop
            localStorage.removeItem('chat_sess_id'); sessionId = null;
            addMsg('bot', getEmergencyResponse(), [], null, localActions); 
        }
    }
